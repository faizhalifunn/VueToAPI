<template>
  <div
    class="min-h-screen flex items-center justify-center bg-black px-4 py-12 relative"
  >
    <!-- Panel Container -->
    <div
      class="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl text-neutral-800 relative"
    >
      ...
      <div class="mb-6">
        <div class="flex flex-col md:flex-row gap-6">
          <div class="flex-1">
            ...
            <div v-for="(item, idx) in forecastRounds" :key="idx" class="mb-3">
              <label class="text-sm block mb-1 text-center"
                >Round {{ idx + 1 }}</label
              >
              <input
                v-model="item.value"
        ...
        <div class="flex justify-between gap-6 mt-6">
          <button
            @click="removeRound"
            :disabled="forecastRounds.length <= 1"
            class="bg-red-500 text-white px-24 py-2 rounded-lg hover:bg-red-600 disabled:opacity-50 ml-12"
          >
            Remove Round
          </button>
          <button
            @click="addRound"
            class="bg-green-500 text-white px-27 py-2 rounded-lg hover:bg-green-600"
          >
            Add Round
          </button>
        </div>
      ...
      <button
        ...
        class="w-full bg-black text-white py-4 rounded-xl text-lg font-semibold hover:bg-gray-800 disabled:opacity-50"
      >
      ...
      >
        <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-sm">
          ...
          <p class="mb-6 text-center">
            Submit interest before starting the game?
          </p>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
          ...
          <div class="flex justify-end mt-6">
            <button
              ...
              class="mr-2 px-4 py-2 bg-gray-300 rounded-lg"
            >
              Cancel
            </button>
            <button
...
const submitInterest = async () => {
  ...
  try {
    await axios.post("https://api-fastify-pi.vercel.app/game/addinterest", {
      ...
      ConsumptiveInterest: interest.value.ConsumptiveInterest,
      ProductiveInterest: interest.value.ProductiveInterest,
      InterOfficeInterest: interest.value.InterOfficeInterest,
      FundInterest: interest.value.FundInterest,
    });

    const started = await startGame();
    if (!started)
      throw new Error(
        "Belum ada tim yang join. Silakan tunggu sebelum memulai game."
      );
...
const startGame = async () => {
  try {
    const response = await fetch(
      "https://api-fastify-pi.vercel.app/round/start",
      {
        method: "POST",
