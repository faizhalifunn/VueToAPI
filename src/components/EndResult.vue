  ...
  <div class="relative">
    ...
    >
      ...
      >
        ...
        <!-- ✅ END SUMMARY (samain seperti screenshot + FinalContributionPoint) -->
        <div v-if="activeTab === 'summary'">
          <div class="border-t border-white/30 overflow-x-auto">
            ...
            <div
              ...
              :style="{ gridTemplateColumns: `repeat(${2 + endSummaryFields.length}, minmax(180px, 1fr))` }"
            >
              <span>No.</span>
              <span>Team</span>
              <span v-for="f in endSummaryFields" :key="'h-'+f.key">{{ f.label }}</span>
            </div>
...
/* =========================
   END SUMMARY (sesuai screenshot) + FinalContributionPoint => TotalBranchValue
   ========================= */
const endSummaryFields = [
  { label: "Active Customers",                        key: "ActiveCustomers",                     type: "nominal" },
  { label: "Active Customers Contribution Bonus",     key: "ActiveCustomersContributionBonus",    type: "nominal" },
...
/* =========================
   KPI SESUAI LIST PM (untuk tab Round & Charts)
   ========================= */
const kpiFields = [
...
const sortedTeams = computed(() => {
  ...
  return Object.entries(teamsObject)
    .map(([teamName, rounds], index) => {
      ...
      if (endResult.baseContributionMargin == null) {
        endResult.baseContributionMargin = Number(endResult.ContributionMargin) || 0;
      }

      // totalAchievementStar fallback dari AchievementStar
      if (endResult.totalAchievementStar == null && endResult.AchievementStar != null) {
        endResult.totalAchievementStar = Number(endResult.AchievementStar) || 0;
      }

      // totalAchievementStarscore default 0 kalau belum ada
      if (endResult.totalAchievementStarscore == null) {
        endResult.totalAchievementStarscore = Number(endResult.totalAchievementStarscore) || 0;
      }

      // HighestRound default pakai "Round 1" kalau kosong
      if (!endResult.HighestRound) {
        const roundsKeys = Object.keys(rounds || {}).filter(k => k.startsWith("Round "));
        const maxRound = roundsKeys
          .map(k => parseInt(k.split(" ")[1]))
          .reduce((a, b) => isNaN(b) ? a : Math.max(a, b), 1);
        endResult.HighestRound = `Round ${maxRound || 1}`;
      }

      // calculatedAt default "-"
      if (!endResult.calculatedAt && gameData.value?.calculatedAt) {
        endResult.calculatedAt = gameData.value.calculatedAt;
      }

      // ===== Fallback ringan untuk KPI totals agar chart/tab lain aman =====
    ...
    .sort((a, b) =>
      (b.endResult.ContributionMargin || b.endResult.baseContributionMargin || 0) -
      (a.endResult.ContributionMargin || a.endResult.baseContributionMargin || 0)
    );
});
